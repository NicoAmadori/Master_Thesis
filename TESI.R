...
plot_residual_distribution_ETF <- function(result, model_name, sector) {
  ...
  hist(standardized_residuals, main = paste("Residui Standardizzati -", sector, model_name),
       xlab = "Residui Standardizzati", col = "azzurro chiaro", border = "nero", probability = TRUE, breaks = 100)
  
  # Aggiungo una curva di densità in verde bosco
  lines(density(standardized_residuals), col = "verdefoglia", lwd = 2)
  ...
  lines(normal_line, normal_density, col = "rosso", lwd = 2)
  
  rug(standardized_residuals, col = "blu")
}
